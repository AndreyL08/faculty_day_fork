...
```bash
find logs -maxdepth 1 -type f -printf "%s %f\n" | sort -nr | head -n 1 | awk '{print $2}' > biglog.txt
```
...
```bash
zip -j config_backup.zip logs/config/*
```
...
```bash
zip project_backup.zip logs/config/*.conf $(find logs -type f \( -name "*.log" -o -name "*.logs" \)) $( [ -f errors.txt ] && echo errors.txt )
```
...
```bash
for f in logs/config/*.conf; do echo "$(basename "$f") $(wc -l < "$f")"; done > conf_stats.txt
```
